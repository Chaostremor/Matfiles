...
% wdwSTAstretchBmom.m:  Tries to get at various moment estimates (amp^2, sqrt(amp^2), coherent, etc.)
...
%   bostname='BOSTOCK/NEW/002-246_2003.062'; 
   ...
   %         'BOSTOCK/NEW/002-246_2005.257'];
    ...
    npo=2;
    npa=2;
    mshift=7; %maximum shift for the x-correlations. 19 for 002 Stanford.  4 for 068?
...
% This is the window SSIB SILB dot product
                    cumcohrad=cumsumSTA23tr(end);
% This is window radiated energy
                    cumrad=realsqrt(ST2sq(end)*ST3sq(end));
% Coherent window amplitude
                    tmp=STA23tr;
                    tmp(STA23tr<0)=0;
                    roottmp=sqrt(tmp);
                    cumroottmp=cumsum(roottmp);
                    cumcohabs=cumroottmp(end);
% Total window amplitude
                    cumabsST2=cumsum(abs(STA2file(winlen*(nt-1)+1:winlen*nt,2)));
                    cumabsST3=cumsum(abs(STA3file(winlen*(nt-1)+1:winlen*nt,2)));
                    cumabs=0.5*(cumabsST2(end)+cumabsST3(end));
                    ...
                    ccdat(nt,5)=mapfile(nt,2);
                    ccdat(nt,6)=mapfile(nt,3);
                    ccdat(nt,7)=Prev(nt);
                    ...
                    ccdat(nt,13)=cumcohrad;
                    ccdat(nt,14)=cumrad;
                    ccdat(nt,15)=cumcohabs;
                    ccdat(nt,16)=cumabs;
...
%     fclose(fid);
    fid = fopen(['ARMMAP/MAPS/2018Oct/ccdat_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
    fprintf(fid,'%5i %4i %4i %11.4e %6.2f %6.2f %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e  %12.3e %10.3e %10.3e %10.3e \n',ccdat(1:nin,:)');
    %fprintf(fid,'%5i %4i %4i %11.4e %4i %4i %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e  %6i %5i %5i  %7i %5i %5i  %8i %6i %6i  %8i %6i %6i \n',ccdat(1:nin,:)'); This has the walk-out Hilbert transforms.
    fclose(fid);
