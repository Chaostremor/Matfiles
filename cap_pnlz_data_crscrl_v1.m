...
totalshiftp = conshift+corshiftp;
...
plot(dist, totalshiftp, 'b.', 'MarkerSize', 20);
set(gca, 'YLIM', [-8, 8]);
set(gca, 'YTick', -8: 2: 8);
...
stnmuse = [];
for i = 1:sa
    if totalshiftp(i) > -5 && totalshiftp(i) < 0
        distuse =  [distuse; dist(i)];
        shiftuse = [shiftuse; totalshiftp(i)];
        pguse = [pguse; realpg(i)];
%         sguse = [sguse; realsg(i)];
        stnmuse = [stnmuse; stnm(i, :)];
    end
