...
% fid2 = fopen('G:\Alxa\test3_13\timeshiftfromcap35_13') ;      % strcat用于字符串连接
% capshift = textscan(fid2, '%s %f %f %f\n') ;  % 得到的是一个cell array,调用语句为flielist{1}  
% %
% fclose(fid2) ;
% conshift = capshift{2};     % 台站名是第一列
% corshiftp = capshift{3};
% corshifts = capshift{4};
% totalshifts = conshift+corshifts;
...
%     i=60;
    ...
    t4use(1: nptuse(i), i) = t2use(1: nptuse(i), i) + tlag(i) + conshift(i);
...
% % totalshifts = -abs(totalshifts);
% plot(dist, totalshifts, 'b.', 'MarkerSize', 20);
% % set(gca, 'YLIM', [-8, 8]);
% % set(gca, 'YTick', -8: 2: 8);
...
% %%%%%%%%%%% 2. use cap crscrl to do pg correction
% realsg = synsg+totalshifts;
% distuse = [];
% shiftuse = [];
% sguse = [];
% sguse = [];
% stnmuse = [];
% for i = 1:sa
%     if totalshifts(i) > -5 && totalshifts(i) < 0
%         distuse =  [distuse; dist(i)];
%         shiftuse = [shiftuse; totalshifts(i)];
%         sguse = [sguse; realsg(i)];
%         sguse = [sguse; realsg(i)];
%         stnmuse = [stnmuse; stnm(i, :)];
%     end
...
% figure
% % plot(dist(ind), totalshift(ind), 'b.', 'MarkerSize', 20);
% plot(distuse, shiftuse, 'b.', 'MarkerSize', 20);
% set(gca, 'YLIM', [-8, 8]);
% set(gca, 'YTick', -8: 2: 8);
